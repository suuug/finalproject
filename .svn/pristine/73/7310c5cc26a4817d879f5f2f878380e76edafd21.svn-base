...
<script>
	$(document).ready(
			function() {
				...
				if (pos_pos != '') {
					$("#selectForm > div:nth-child(1) > label").html(
							$('option[id=' + pos_pos + ']').html());
				}

				if (dep_dep != '') {
					console.log("check",
							$("#selectForm > div:nth-child(2) > label"));
					$("#selectForm > div:nth-child(2) > label").html(
							$('option[id=' + dep_dep + ']').html());
				}
				...
				selectTarget.change(function() {
					var select_name = $(this).children('option:selected')
							.text();
			...
			<div class="card">
				<div class="card-body">
					<div class="row">
						...
						<form name="selectForm" id="selectForm" action="/human/list"
							method="get">
					<div class="selectbox col-sm-3">
								<label for="ex_select">직책선택</label> 
					...
					<div class="selectbox col-sm-3">
								<label for="ex_select">부서선택</label> 								
								<select id="ex_select"
									name="dep" onchange="se1_submit(this)">
									...
									<tbody>
										<c:forEach var="row" items="${list}">
											<tr>
												...
												<td>${row.deptName}</td>
												<td><fmt:formatDate value="${row.empEcnyYmd}"
														type="date" /></td>
												<td>${row.empEmail}</td>
											</tr>
										</c:forEach>
