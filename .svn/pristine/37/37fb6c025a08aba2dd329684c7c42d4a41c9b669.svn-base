...
<!--
 ...
 -->
<form:form modelAttribute="boardVO" method="post" id="update_form"
			...
			<div class="card">
				<div class="card-body">
					...
					<div class="row mb-3">
						...
						<form:input type="hidden" path="atchId" value="0"></form:input>
					</div>
				</div>
			</div>
		</div>
								...
								<c:forEach items="${files}" var="file" varStatus="idx" step="1">
									<tr>
										...
										<td><form:button type="button"
												onclick="$('#atchId').val('${file.atchId }');$('#update_form').submit()"
