...
<div class="row m-1">
	<h3 class="h3 mb-2 text-gray-800" style="color: #6c6ff5">받은 결재함</h3>
</div>
...
<div class="row m-1">
	<div class="card overflow-hidden card-h-100 ">
		<div class="card-body" id="j-card">
			...
			<form:form modelAttribute="atrzVO" action="/autho/receiveDocSuccess" method="post">
				<div class="row mb-3">
					<div class="col d-flex flex-row-reverse bd-highlight">
						<button type="button" class="btn btn-danger ms-2" id="j-cancelBtn" style="width:10%;">뒤로가기</button>
						<c:if test="${loginAtrzTypeId == '대기중'}">
							<button type="button" class="btn btn-secondary ms-2 j-formBtn" style="width:15%;" value="반려">반려</button>
							<button type="button" class="btn btn-primary j-formBtn" style="width:15%;" value="승인">승인</button>
						</c:if>
					</div>
				</div>
				<sec:csrfInput/>
				<form:hidden path="docId" value="${atrzDocVO.docId }"/>
				<form:hidden path="atrzTypeId" value=""/>
				<input type="hidden" name="docWriter" value="${atrzDocVO.docWrtr}"/>
			</form:form>
		</div>
	</div>
</div>

<p id="username" style="display:none"><sec:authentication property='principal.user.username'/></p>

<script type="text/javascript">
$(function(){
	var v_loginSign = "${loginSign}";
	$("#j-cancelBtn").on("click",function(){
		location.href = "/autho/receiveDoc";
	})
	$(".j-formBtn").on("click",function(){
		var v_btnVal = $(this).val();
		if(v_loginSign == "SIGN00000" && v_btnVal == "승인"){
			alert("서명을 먼저 등록해주세요.");
			location.href="/autho/setting";
			return false;
		}
		$("#atrzTypeId").val(v_btnVal);
		$("form").submit();
	})
})
</script>